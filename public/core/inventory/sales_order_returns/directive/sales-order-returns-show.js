salesOrderReturns.directive('salesOrderReturnsShow', function () {
    return {
        restrict: 'E',
        scope: {
        ...
        require:'ngModel',
        templateUrl: '/core/inventory/sales_order_returns/view/purchase-requests-show.html',
  ...
  $scope.show = function() {
      ...
      $scope.editForm=false;
      $scope.price_po=false;
      $http.get(baseUrl+'/inventory/sales_order_return/'+$stateParams.id).then(function(data) {
    ...
    $scope.backward = function() {
        if($scope.index_route) {
            ...
            if($rootScope.hasBuffer()) {
                ...
                $state.go('inventory.sales_order_return')
            }
        }
    }
  ...
  $scope.approveSubmit=function() {
    ...
    $http.post(baseUrl+'/inventory/sales_order_return/approve/'+$stateParams.id+'?_token='+csrfToken,$scope.formData).then(function(data) {
        $rootScope.disBtn=false;
        $scope.show()
    });
  }
  $scope.deleteDetail=function(id) {
    ...
    $rootScope.disBtn=true;
    $http.delete(baseUrl+'/inventory/sales_order_return/delete_detail/'+id).then(function(data) {
  ...
  $scope.rejectSubmit=function() {
    $rootScope.disBtn=true;
    $http.post(baseUrl+'/inventory/sales_order_return/reject/'+$stateParams.id,$scope.rejectData).then(function(data) {
  ...
  $scope.submitDetail=function() {
    $rootScope.disBtn=true;
    $http.post(baseUrl+'/inventory/sales_order_return/store_detail/'+$scope.detailData.id,$scope.detailData).then(function(data) {
        $rootScope.disBtn=false;
  ...
  $scope.poSubmit=function() {
    $rootScope.disBtn=true;
    $http.post(baseUrl+'/inventory/sales_order_return/create_po/'+$stateParams.id+'?_token='+csrfToken,$scope.formData).then(function(data) {
      $rootScope.disBtn=false;
      $scope.show()
    }, function(error) {
        $rootScope.disBtn=false;

stocklistSales.controller('stocklistSalesShow', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile, stocklistSalesService) {
    $rootScope.pageTitle = $rootScope.solog.label.sales_order.title;
    $scope.formData = {}
    $scope.isShipment = true
    
    $scope.show = function() {
        stocklistSalesService.api.show($stateParams.id, function(dt) {
            $scope.formData = dt

stockByItemSales.controller('stockByItemSalesShow', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile, stockByItemSalesService) {
    $rootScope.pageTitle = $rootScope.solog.label.sales_order.title;
    $scope.formData = {}
    $scope.isShipment = true
    
    $scope.show = function() {
        stockByItemSalesService.api.show($stateParams.id, function(dt) {
            $scope.formData = dt

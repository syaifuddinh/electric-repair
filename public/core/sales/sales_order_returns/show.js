salesOrderReturnSales.controller('salesOrderReturnSalesShow', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile, salesOrderReturnSalesService) {
    $rootScope.pageTitle = $rootScope.solog.label.sales_order.title;
    $scope.formData = {}
    $scope.isShipment = true
    
    $scope.show = function() {
        salesOrderReturnSalesService.api.show($stateParams.id, function(dt) {
            $scope.formData = dt

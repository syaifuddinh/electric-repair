...
invoiceSales.controller('invoiceSalesShow', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile, invoiceSalesService) {
    $rootScope.pageTitle = $rootScope.solog.label.sales_order.title;
    $scope.formData = {}
    $scope.isShipment = true
    
    $scope.show = function() {
        invoiceSalesService.api.show($stateParams.id, function(dt) {
            $scope.formData = dt

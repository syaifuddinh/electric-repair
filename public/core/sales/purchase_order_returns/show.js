purchaseOrderReturnSales.controller('purchaseOrderReturnSalesShow', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile, purchaseOrderReturnSalesService) {
    $rootScope.pageTitle = $rootScope.solog.label.sales_order.title;
    $scope.formData = {}
    $scope.isShipment = true
    
    $scope.show = function() {
        purchaseOrderReturnSalesService.api.show($stateParams.id, function(dt) {
            $scope.formData = dt

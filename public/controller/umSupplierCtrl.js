app.controller('financeUmSupplier', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile) {
  $rootScope.pageTitle="Deposit Supplier";
  $scope.formData = {};
  ...
  oTable = $('#datatable').DataTable({
    ...
    ajax : {
      ...
      url : baseUrl+'/api/finance/um_supplier_datatable',
      data : function(request) {
        ...
        request['supplier_id'] = $scope.formData.supplier_id;
    ...
    createdRow: function(row, data, dataIndex) {
      if($rootScope.roleList.includes('finance.deposite.supplier.detail')) {
        $(row).find('td').attr('ui-sref', 'finance.um_supplier.show({id:' + data.id + '})')
  ...
  $scope.deletes=function(ids) {
    ...
    if (cfs) {
      $http.delete(baseUrl+'/finance/um_supplier/'+ids,{_token:csrfToken}).then(function success(data) {
        // $state.reload();
        oTable.ajax.reload();
  ...
  $scope.exportExcel = function() {
    ...
    var url = baseUrl + '/excel/um_supplier_export?';
    url += params;
    location.href = url; 
  }
});

app.controller('financeUmSupplierCreate', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile,$filter) {
  $rootScope.pageTitle="Tambah Deposit Supplier";
  $('.ibox-content').addClass('sk-loading');
  ...
  $scope.modalCekGiro=function() {
    if ($scope.formData.contact_id) {
      ...
      toastr.warning("Supplier Belum Dipilih!");
    }
  }
  ...
  oTable = $('#cekGiroDatatable').DataTable({
    ...
    ajax : {
      ...
      data:function(d) {
        d.penerima_id=$scope.formData.contact_id;
        d.is_used=0;
      }
    },
    order:[[2,'asc']],
    columns:[
      ...
      {data:"penerbit.name",name:"penerbit.name",className:"hidden"},
      {data:"penerima.name",name:"penerima.name",className:"hidden"},
  ...
  $http.get(baseUrl+'/finance/um_supplier/create').then(function(data) {
    $scope.data=data.data;
    $('.ibox-content').removeClass('sk-loading');
  });
  ...
  $scope.submitForm=function() {
    ...
    $.ajax({
      type: "post",
      url: baseUrl+'/finance/um_supplier?_token='+csrfToken,
      data: $scope.formData,
...
app.controller('financeUmSupplierShow', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile,$filter) {
    $rootScope.pageTitle="Detail Deposit Supplier";
    $('.ibox-content').addClass('sk-loading');
    $scope.total_bayar=0;
    
    $http.get(baseUrl+'/finance/um_supplier/'+$stateParams.id).then(function(data) {
        $scope.data=data.data;
        $scope.total=data.data.item.debet;
        ...
        $scope.sisa =  $scope.data.item.debet - $scope.data.item.credit;
        $('.ibox-content').removeClass('sk-loading');
    });
    ...
    $scope.simpanKembali = function() {
        ...
        $scope.disBtn = true;
        $http.post(baseUrl+'/finance/um_supplier/'+$stateParams.id+'/return_sisa', $scope.formData)
            .then(function(data) {

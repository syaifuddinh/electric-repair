app.controller('financeBillPayment', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile) {
  $rootScope.pageTitle="Penerimaan Pembayaran";
  $scope.formData = {};
  ...
  oTable = $('#datatable').DataTable({
    ...
    ajax: {
      ...
      url : baseUrl+'/api/finance/bill_payment_datatable',
...
app.controller('financeBillPaymentShow', function($scope, $http, $rootScope,$state,$stateParams,$timeout,$compile,$filter) {
  ...
  $scope.validBP=function(i) {
    ...
    $http.post(baseUrl+'/finance/bill_receivable/validasiBP/'+$stateParams.id,$scope.formData.paymentbp).then(function(data) {
      // $state.go('finance.bill_receivable.show',{id:$stateParams.id});
      $state.go('finance.bill_payment.show',{id:$stateParams.id});
      $state.reload();
  ...
  $scope.submitFormny=function(dom) {
    ...
    $.ajax({
      url:baseUrl+'/finance/bill_receivable/uploadBP/'+$scope.buktipotong+'?_token='+csrfToken,
      contentType : false,
      processData : false,
      type : 'POST',
      data : fd,
